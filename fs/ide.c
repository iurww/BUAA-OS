...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_read_dev((void *)&temp, (DEV_DISK_ADDRESS + DEV_DISK_STATUS), 4));
		panic_on(temp == 0);

		panic_on(syscall_read_dev((void *)(dst + off), (DEV_DISK_ADDRESS + DEV_DISK_BUFFER), 512));
	}
}
...
void ide_write(u_int diskno, u_int secno, void *src, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_write_dev((void *)(src + off), (DEV_DISK_ADDRESS + DEV_DISK_BUFFER), 512));
		...
		panic_on(
		    syscall_write_dev((void *)&temp, (DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION), 4));
